...
Item {
  ...
  function applyRecoveryCommand(command) {
    if (!ready || !recordingReady || saving) return false
    try {
      var result = RecoveryModel.decide(recoveryProjection, command)
      if (result.events.length === 0) return true
      var now = new Date()
      var localContext = EventModel.localSystemContext(now, systemTimezone)
      ...
      errorMessage = "Could not update DailyXP recovery: " + error
      console.warn("dailyxp/recovery", errorMessage)
      return false
    }
  }
